...
public class q4PortalListener extends AbstractPortalListener {

    public q4PortalListener(MyDungeonTeleportPlugin plugin) {
        super(plugin, "q4", -1026, -1015, -60, -48, -566, -566);
    }
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (DungeonUtils.isInPortalArea(loc, x1, x2, y1, y2, z1, z2)) {
            ...
            if (selectedMap.equals("q4_m1_inf") && playerLevel >= 50) {
                requiredIPS = 10;
                plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "warp q4_m1_inf " + player.getName());
                plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "mm s resettimers g:q4_inf");
                plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "quests start " + player.getName() + " 81 -overrideRequirements");
                taskId = scheduleTimeoutTask(player);
                player.sendTitle(ChatColor.GOLD + "Q4 Quest Started", ChatColor.YELLOW + "You have 30 minutes to clear it. Good luck!", 10, 70, 20);
            } else if (selectedMap.equals("q4_m1_hell") && playerLevel >= 65) {
                requiredIPS = 25;
                plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "warp q4_m1_hell " + player.getName());
                plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "mm s resettimers g:q4_hell");
                plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "quests start " + player.getName() + " 82 -overrideRequirements");
                taskId = scheduleTimeoutTask(player);
                player.sendTitle(ChatColor.GOLD + "Q4 Quest Started", ChatColor.YELLOW + "You have 30 minutes to clear it. Good luck!", 10, 70, 20);
            } else if (selectedMap.equals("q4_m1_blood") && playerLevel >= 80) {
                requiredIPS = 50;
                plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "warp q4_m1_blood " + player.getName());
                plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "mm s resettimers g:q4_blood");
                plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "quests start " + player.getName() + " 83 -overrideRequirements");
                taskId = scheduleTimeoutTask(player);
                player.sendTitle(ChatColor.GOLD + "Q4 Quest Started", ChatColor.YELLOW + "You have 30 minutes to clear it. Good luck!", 10, 70, 20);
            } else {
