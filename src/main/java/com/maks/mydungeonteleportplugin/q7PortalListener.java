...
public class q7PortalListener extends AbstractPortalListener {

    public q7PortalListener(MyDungeonTeleportPlugin plugin) {
        super(plugin, "q7", -778, -778, -60, -48, -375, -364);
    }
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (DungeonUtils.isInPortalArea(loc, x1, x2, y1, y2, z1, z2)) {
            ...
            if (selectedMap.equals("q7_m1_inf") && playerLevel >= 50) {
                requiredIPS = 10;
                plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "warp q7_m1_inf " + player.getName());
                plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "mm s resettimers g:q7_inf");
                plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "quests start " + player.getName() + " 111 -overrideRequirements");
                taskId = scheduleTimeoutTask(player);
                player.sendTitle(ChatColor.GOLD + "Q7 Quest Started", ChatColor.YELLOW + "You have 30 minutes to clear it. Good luck!", 10, 70, 20);
            } else if (selectedMap.equals("q7_m1_hell") && playerLevel >= 65) {
                requiredIPS = 25;
                plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "warp q7_m1_hell " + player.getName());
                plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "mm s resettimers g:q7_hell");
                plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "quests start " + player.getName() + " 112 -overrideRequirements");
                taskId = scheduleTimeoutTask(player);
                player.sendTitle(ChatColor.GOLD + "Q7 Quest Started", ChatColor.YELLOW + "You have 30 minutes to clear it. Good luck!", 10, 70, 20);
            } else if (selectedMap.equals("q7_m1_blood") && playerLevel >= 80) {
                requiredIPS = 50;
                plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "warp q7_m1_blood " + player.getName());
                plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "mm s resettimers g:q7_blood");
                plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "quests start " + player.getName() + " 113 -overrideRequirements");
                taskId = scheduleTimeoutTask(player);
                player.sendTitle(ChatColor.GOLD + "Q7 Quest Started", ChatColor.YELLOW + "You have 30 minutes to clear it. Good luck!", 10, 70, 20);
            } else {
